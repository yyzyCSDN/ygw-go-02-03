...
func EncodeJSON(values []model.Item) ([]byte, error) {
	cloned := make([]model.Item, len(values))
	for index, value := range values {
		cloned[index] = value.Clone()
	}
	data, err := json.MarshalIndent(cloned, "", "  ")
	if err != nil {
		return nil, fmt.Errorf("encode item list: %w", err)
	}
	return append(data, '\n'), nil
}

func DecodeJSON(reader io.Reader) ([]model.Item, error) {
...
func EncodeCSV(values []model.Item) ([]byte, error) {
	...
	for _, value := range values {
		...
		if err := writer.Write(row); err != nil {
			return nil, fmt.Errorf("write item CSV: %w", err)
		}
	}
	writer.Flush()
	if err := writer.Error(); err != nil {
		return nil, fmt.Errorf("flush item CSV: %w", err)
	}
	return buffer.Bytes(), nil
}

func DecodeCSV(reader io.Reader) ([]model.Item, error) {
	rows, err := csv.NewReader(reader).ReadAll()
	if err != nil {
		return nil, fmt.Errorf("read item CSV: %w", err)
	}
	if len(rows) == 0 {
	...
	result := make([]model.Item, 0, len(rows)-1)
	for rowNumber, row := range rows[1:] {
		...
		value := model.Item{ID: row[0], Name: row[1], Status: row[2], Priority: priority, Amount: amount, Active: active, Version: version, Tags: strings.Split(row[7], "|"), UpdatedAt: updated}
