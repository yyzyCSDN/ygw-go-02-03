...
import (
	...
	"example.com/inventoryreplenisher/model"
)
...
type PredicateFunc struct {
	Name string
	Test func(model.Item) bool
}

func (predicate PredicateFunc) Match(value model.Item) bool {
...
func (predicates All) Match(value model.Item) bool {
...
func (predicates Any) Match(value model.Item) bool {
	for _, predicate := range predicates {
		if predicate != nil && predicate.Match(value) {
			return true
		}
...
func (predicate Not) Match(value model.Item) bool {
...
func Active() Predicate {
	return PredicateFunc{Name: "active", Test: func(value model.Item) bool { return value.Active }}
}

func Status(status string) Predicate {
	want := strings.ToLower(strings.TrimSpace(status))
	return PredicateFunc{Name: "status=" + want, Test: func(value model.Item) bool {
...
func Tagged(tag string) Predicate {
	...
	return PredicateFunc{Name: "tag=" + want, Test: func(value model.Item) bool { return value.HasTag(want) }}
}

func MinimumAmount(minimum int64) Predicate {
	return PredicateFunc{Name: fmt.Sprintf("amount>=%d", minimum), Test: func(value model.Item) bool {
		return value.Amount >= minimum
	}}
}

func PriorityBetween(minimum, maximum int) Predicate {
	return PredicateFunc{Name: fmt.Sprintf("priority=%d..%d", minimum, maximum), Test: func(value model.Item) bool {
...
func (set *Set) Evaluate(value model.Item) Evaluation {
...
func (set *Set) Select(values []model.Item, name string) ([]model.Item, error) {
	if set == nil {
