...
func TestItemCloneAndTags(t *testing.T) {
	original := Item{ID: "one", Name: "First", Tags: []string{"Blue"}, Metadata: map[string]string{"zone": "a"}}
...
func TestItemValidationAndTouch(t *testing.T) {
	if err := (Item{}).Validate(); err == nil {
		t.Fatal("expected validation error")
	}
	now := time.Unix(100, 0).UTC()
	value := (Item{ID: "one", Name: "First"}).Touch(now)
