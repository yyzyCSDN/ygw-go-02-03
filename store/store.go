...
func New(historyLimit int) *Store {
	...
	return &Store{clock: time.Now, items: map[string]model.Item{}, limit: historyLimit}
}

func (store *Store) Put(value model.Item) (model.Item, error) {
...
func (store *Store) Create(value model.Item) (model.Item, error) {
	store.mu.RLock()
	_, exists := store.items[value.ID]
	store.mu.RUnlock()
	if exists {
		return model.Item{}, fmt.Errorf("item %q already exists", value.ID)
	}
	return store.Put(value)
}

func (store *Store) Get(id string) (model.Item, bool) {
	store.mu.RLock()
...
func (store *Store) Delete(id string) (model.Item, error) {
	store.mu.Lock()
	defer store.mu.Unlock()
	value, exists := store.items[id]
	if !exists {
		return model.Item{}, fmt.Errorf("item %q does not exist", id)
	}
...
func (store *Store) List() []model.Item {
	store.mu.RLock()
	defer store.mu.RUnlock()
	result := make([]model.Item, 0, len(store.items))
...
func (store *Store) FindByName(text string) []model.Item {
...
func (store *Store) ReplaceAll(values []model.Item) error {
	replacement := make(map[string]model.Item, len(values))
	now := store.clock()
	for _, value := range values {
		...
		if _, duplicate := replacement[value.ID]; duplicate {
			return fmt.Errorf("duplicate item id %q", value.ID)
		}
