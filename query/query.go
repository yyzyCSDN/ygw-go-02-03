...
func Select(values []model.Item, filter Filter) []model.Item {
...
func Sort(values []model.Item, field SortField, descending bool) ([]model.Item, error) {
	result := make([]model.Item, len(values))
...
func Page(values []model.Item, offset, limit int) ([]model.Item, error) {
	if offset < 0 || limit < 0 {
	...
	result := make([]model.Item, end-offset)
...
func GroupByStatus(values []model.Item) map[string][]model.Item {
	result := map[string][]model.Item{}
	for _, value := range values {
...
func IDs(values []model.Item) []string {
