...
func Apply(current []model.Item, operations []Operation) (Result, error) {
	working := map[string]model.Item{}
	...
	for index, operation := range operations {
		...
		case Delete:
			if _, exists := working[operation.ID]; !exists {
				return Result{}, fmt.Errorf("operation %d: item %q does not exist", index+1, operation.ID)
			}
	...
	sort.Strings(ids)
	result.Values = make([]model.Item, 0, len(ids))
	for _, id := range ids {
...
func ValidateUnique(values []model.Item) error {
