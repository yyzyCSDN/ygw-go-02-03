...
func TestApplyOperations(t *testing.T) {
	current := []model.Item{{ID: "one", Name: "One"}, {ID: "two", Name: "Two"}}
	result, err := Apply(current, []Operation{
		{Kind: Upsert, Value: model.Item{ID: "one", Name: "Updated"}},
		{Kind: Delete, ID: "two"},
		{Kind: Upsert, Value: model.Item{ID: "three", Name: "Three"}},
	})
	if err != nil {
		t.Fatal(err)
	}
...
func TestApplyFailureReturnsNoPartialResult(t *testing.T) {
	result, err := Apply([]model.Item{{ID: "one", Name: "One"}}, []Operation{
		{Kind: Upsert, Value: model.Item{ID: "two", Name: "Two"}},
	...
	if err := ValidateUnique([]model.Item{{ID: "x"}, {ID: "x"}}); err == nil {
